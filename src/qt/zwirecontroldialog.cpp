...
#include "ui_zwirecontroldialog.h"
...
std::list<std::string> ZWireControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZWireControlDialog::listMints;

ZWireControlDialog::ZWireControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZWireControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZWireControlDialog::~ZWireControlDialog()
{
    delete ui;
}

void ZWireControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZWireControlDialog::updateList()
{
...
void ZWireControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZWireControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZWire_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZWireControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZWireControlDialog::GetSelectedMints()
{
...
void ZWireControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
