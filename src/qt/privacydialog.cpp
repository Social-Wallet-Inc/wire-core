...
void PrivacyDialog::on_pushButtonZWireControl_clicked()
{
    ZWireControlDialog* zWireControl = new ZWireControlDialog(this);
    zWireControl->setModel(walletModel);
    zWireControl->exec();
}

void PrivacyDialog::setZWireControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzWireSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzWIRE()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Wire Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // use mints from zWire selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZWireControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZWireControlDialog::GetSelectedMints();
    }
    ...
    // Clear zwire selector in case it was used
    ZWireControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zWire Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Wire, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zWire Mint");
